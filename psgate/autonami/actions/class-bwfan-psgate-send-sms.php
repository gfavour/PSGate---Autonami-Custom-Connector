...
class BWFAN_Psgate_Send_SMS extends BWFAN_Action {
	...
	public function __construct() {
		...
		$this->action_desc = __( 'This action sends a SMS via Psgate', 'autonami-automations-connectors' );
	}

	/**
	 * @return BWFAN_Psgate_Send_SMS|null
	 */
		...
		<script type="text/html" id="tmpl-action-<?php echo esc_attr__( $unique_slug ); ?>">
			...
			<div data-element-type="bwfan-editor" class="bwfan-<?php echo esc_attr__( $unique_slug ); ?>">
				...
				<div class="bwfan-col-sm-12 bwfan-pl-0 bwfan-pr-0 bwfan-mt-10 bwfan-mb-10 bwfan_attach_custom_img_box {{defVal === 3 ? '' : 'hidden'}}" style="margin-top:10px;margin-bottom:10px">
					<div class="form-group bwfan-psgate-uploader">
						<#
				...
				<div class="bwfan-col-sm-12 bwfan-pl-0 bwfan-pr-0 bwfan-mb-15">
					<div class="clearfix bwfan_field_desc">
						<?php esc_html_e( 'Images can only be send to US &amp; Canada phone numbers by using a Psgate phone number that is MMS-enabled.', 'wp-marketing-automations' ); ?><br/>
						<a href="https://support.psgate.com/hc/en-us/articles/223179808-Sending-and-receiving-MMS-messages" target="_blank"><?php esc_html_e( 'Click here', 'wp-marketing-automations' ); ?></a>
		...
		<script>
			...
			jQuery(document).on('click', '#bwfan_media_manager', function (e) {
				...
				if (_.size(_.filter(cus_img_url)) > 0) {
					imgUpload = false;
					$this.parents('.bwfan-psgate-uploader').append('<p class="bwfan_img_notice">Note: Only one image is allowed.</p>');
					setTimeout(function () {
	...
	public function execute_action( $action_data ) {
		...
		$call_class     = $load_connector->get_call( 'wfco_psgate_send_sms' );
		...
		$integration  = BWFAN_Psgate_Integration::get_instance();
		$this->data['username'] = $integration->get_settings( 'username' );
		$this->data['password']  = $integration->get_settings( 'password' );
		$this->data['psgate_no']   = $integration->get_settings( 'psgate_no' );
		...
		$this->data['sms_body'] = BWFAN_Connectors_Common::modify_sms_body( $this->data['sms_body'], $this->data );

		$call_class->set_data( $this->data );
		$response = $call_class->process();
		do_action( 'bwfan_sendsms_action_response', $response, $this->data );
		
		if ("ok" === strtolower($response['status']) || 200 === $response['status'] || 1701 === $response['status'] || 1701 === $response || $response) {
